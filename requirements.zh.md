...
我们为自己建立了一个雄心勃勃的目标: 了解如何从根本上建立一个计算机网络. 我们的方法来完成这一目标将是从第一原理出发,然后问这样的问题我们自然会问如果建立一个实际的网络. 在每一步中,我们将使用今天的协议来说明不同的设计选择提供给我们,但我们不会接受现有的这些文物的福音. 相反,我们会问 (并回答) 这个问题. *为什么?*网络是按照他们的方式设计的. 虽然它试图解决 仅仅理解它的工作方式的今天,它是认识的基本概念,重要是因为网络不断随着技术的发展和新的应用改变了. 我们的经验是,一旦你了解了基本的想法,任何新的协议,你面对会比较容易消化. 

## 视角

正如我们上面提到的,网络的学生可以从多个角度出发. 我们写这本书的第一版时,大部分的人口都上网,和那些没有得到它,而在工作中,在大学,或通过拨号调制解调器在家. 这套流行的应用程序可以用手指来计算. 因此,像当时大多数的书籍,我们专注于人的设计的网络设备和协议的角度. 我们继续关注这个角度来看,我们的希望是,在读这本书,你将知道如何设计未来的网络设备和协议. 但是,我们也要支付另外两组是越来越重要的观点: 那些开发网络应用程序和那些管理或经营网络. 让我们考虑一下这三个小组如何列出他们对网络的需求: 

-   安*应用程序员*他或她会列出应用程序需要的服务: 例如,保证每条消息的应用程序将要交付没有误差一定的时间或开关的网络之间的不同的连接在用户移动能力. 

-   一*网络运算符*将列表中的一个系统,易于使用和管理的特点: 例如,在故障可以很容易地分离,新的设备可以被添加到网络配置正确,并且很容易解释用法. 

-   一*网络设计师*将列出具有成本效益的设计的属性: 例如,网络资源被有效利用并且公平地分配给不同的用户. 绩效问题也很重要. 

本节试图将这些不同的观点提炼成驱动网络设计的主要考虑事项的高级介绍,并在此过程中识别本书其余部分所处理的挑战. 

## 可伸缩连通性

从显而易见的情况开始,网络必须提供一组计算机之间的连通性. 有时只需要连接几个选择的机器就可以建立一个有限的网络. 事实上,由于隐私和安全的原因,许多私有 (公司) 网络都有明确的目标,即限制连接的机器集. 相比之下,其他的网络 (其中互联网是最好的例子) 被设计成以一种允许它们连接世界上所有计算机的潜力的方式增长. 被设计为支持任意大尺寸的生长的系统被称为*规模*. 以互联网为模型,这本书解决了可伸缩性的挑战. 

### 链接ㄡ节点和云

为了更全面地理解连接的要求,我们需要更仔细地研究计算机在网络中的连接方式. 连接性发生在许多不同的层次上. 在最低级别上,网络可以由两台或更多台计算机组成,这些计算机直接由一些物理介质连接,例如同轴电缆或光纤. 我们称之为物理介质A. *链接*我们经常提到它连接的计算机. *结点*.  (有时,节点是硬件中更专门的部分,而不是计算机,但是出于讨论的目的,我们忽略了这种区别. ) [图1](#direct)物理链路有时局限于一对节点 (这样的链路被称为*点对点*,而在其他情况下,多于两个节点可以共享单个物理链路 (这样的链路被称为*多路存取*) 无线链路,如蜂窝网络和Wi-Fi网络提供的链路,是越来越重要的一类多址链路. 通常情况下,多址链路的大小是有限的,就其可以覆盖的地理距离和可以连接的节点数量而言. 
...
如果计算机网络局限于所有节点通过公共物理介质彼此直接连接的情况,那么要么网络可以连接的计算机数量非常有限,要么从每个节点的后端出来的线路数量将很快bECOME既难管理又非常昂贵. 幸运的是,两个节点之间的连接并不一定意味着它们之间的直接物理连接ℴℴ可以在一组协作节点之间实现间接连接. 考虑下面两个例子,说明如何可以间接地连接计算机的集合. 

[图2](#psn)显示一对节点的集合,每个节点都连接到一个或多个点到点链接. 连接到至少两个链路的那些节点运行软件,将在一个链路上接收到的数据转发到另一个链路上. 如果以系统的方式组织,这些转发节点形成*交换网络*. 有许多类型的交换网络,其中两种最常见的是*电路交换*和*分组交换*. 前者最值得注意的是被电话系统使用,而后者用于绝大多数计算机网络,并将是本书的重点. (然而,电路交换正在光网络领域有所恢复,随着对网络容量的需求不断增长,这变得很重要. )分组交换网络的重要特征是这样的网络中的节点向每个ot发送离散的数据块. 她. 将这些数据块视为与某一应用程序数据 (如文件ㄡ电子邮件或图像) 相对应的数据块. 我们将每个数据块称为*小包裹*或A*消息*现在我们可以互换使用这些术语. 
...
分组交换网络通常使用称为*存储转发*. 顾名思义,存储转发网络中的每个节点首先通过某个链路接收完整的分组,将分组存储在其内部存储器中,然后将完整的分组转发给下一个节点. 相反,电路交换网络首先在一系列链路上建立专用电路,然后允许源节点通过该电路向目的节点发送比特流. 在计算机网络中使用分组交换而不是电路交换的主要原因是效率,将在下一小节讨论. 

云中[图2](#psn)区分内部的节点*实施*网络 (通常称为网络) *开关*,它们的主要功能是存储和转发数据包和云外部的节点*使用*网络 (通常称为网络) *宿主*,它们支持用户并运行应用程序. 还要注意,云计算是计算机网络最重要的图标之一. 通常,我们使用云来表示任何类型的网络,无论是单点到点链路ㄡ多址链路还是交换网络. 因此,无论何时您看到一个图中使用的云,您都可以将它看作本书中涉及的任何网络技术的占位符. 

> 有趣的是,以这种方式使用云早于这个术语. *云计算*至少有几十年,但这两种用法之间有联系,我们稍后再讨论. 
...
第二种方式是一组计算机可以间接连接. [图3](#internet-cloud). 在这种情况下,一组独立的网络 (云) 相互连接,形成一个独立的网络. *互联网络*或者简称互联网. 我们采用因特网的惯例,将网络的通用网络称为小写. *我*互联网和当前运营的TCP/IP互联网作为资本*我*互联网. 连接到两个或多个网络的节点通常称为*路由器*或*网关*,它起到与从一个网络转发消息到另一个网络的转换的作用. 请注意,互联网本身可以被视为另一种网络,这意味着互联网可以由一种互联网来构建. 因此,我们可以递归地通过互连云来构建任意大的网络,以形成更大的云. 可以合理地论证,这种互连大不相同的网络的想法是因特网的基本创新,因特网成功发展到全球规模和数十亿个节点是早期因特网架构师一些非常好的设计决策的结果. TS,我们稍后再讨论. 

仅仅因为一组主机彼此直接或间接连接,并不意味着我们已经成功地提供了主机到主机的连接. 最终的要求是每个节点必须能够说出它想要与之通信的网络上的其他节点中的哪个节点. 这是通过分配一个*地址*到每个节点. 地址是标识节点的字节字符串;也就是说,网络可以使用节点的地址将其与连接到网络的其他节点区分开来. 当源节点希望网络向某个目的地节点传递消息时,它指定目的地节点的地址. 如果发送节点和接收节点没有直接连接,则网络的交换机和路由器使用该地址来决定如何将消息转发到目的地. 系统地确定如何根据其地址向目的节点转发消息. *路由*.

对寻址和路由的简要介绍假定源节点希望向单个目标节点发送消息 (*单播*) 虽然这是最常见的情况,但也可能源节点可能要*广播*消息到网络上的所有节点. 或者,源节点可能希望将消息发送到其他节点的一些子集,但不是全部. *组播*. 因此,除了特定于节点的地址之外,网络的另一个要求是它支持多播和广播地址. 

从这个讨论中去掉的主要思想是我们可以定义一个*网络*递归地由由物理链路连接的两个或多个节点组成,或由一个节点连接的两个或多个网络组成. 换句话说,网络可以由网络的嵌套来构造,其中在底层,网络由一些物理介质实现. 在提供网络连接性方面的关键挑战之一是定义网络上可到达的每个节点的地址 (包括对广播和多播的支持) ,以及使用这些地址将消息转发到适当的目的地节点. 

## 成本有效的资源共享

如上所述,本书着重于分组交换网络. 本节解释计算机网络的关键要求-效率-这导致我们选择分组交换作为策略. 

给定通过网络嵌套间接连接的节点集合,任何一对主机都可以通过一系列链接和节点相互发送消息. 当然,我们不仅要支持一对通信主机,还要为所有对主机提供交换消息的能力. 那么,问题是,所有希望通信的主机如何共享网络,尤其是如果它们希望同时使用它?而且,如果这个问题还不够困难,那么几个主机是如何共享的呢?*链接*当他们都想同时使用它的时候?

要了解主机如何共享网络,我们需要介绍一个基本概念,*多路复用*,这意味着系统资源在多个用户之间共享. 在直观的层次上,多路复用可以通过类比于分时计算机系统来解释,其中单个物理处理器在多个作业之间共享 (多路复用) ,每个作业都认为它具有自己的专用处理器. 类似地,由多个用户发送的数据可以在构成网络的物理链路上复用. 

若要了解这可能如何工作,请考虑[图4](#mux)其中,网络的左侧的三个主机(发送者S1-S3)通过共享仅包含一个物理链路的交换网络向右侧的三个主机(接收者R1-R3)发送数据. (为了简单起见,假设主机S1正在向主机R1发送数据,等等. )在这种情况下,与三对主机对应的三个数据流通过交换机1被复用到单个物理链路上,然后*解复用*通过交换机2返回到单独的流中. 请注意,我们故意模糊了"数据流"对应的内容. 为了便于讨论,假设左侧的每个主机有大量数据要发送给右侧的对应机. 
...
有多种不同的方法将多个流复用到一个物理链路上. 一种常用的方法是*同步时分多路复用* (STDM) . STDM的思想是将时间划分为大小相等的量子,并以循环方式,为每个流提供通过物理链路发送数据的机会. 换句话说,在时间量子1期间,从S1到R1的数据被传输;在时间量子2期间,从S2到R2的数据被传输;在量子3中,S3将数据发送到R3. 此时,第一流 (S1至R1) 再次开始,并且该过程重复. 另一种方法是*频分复用* (FDM) . FDM的思想是在物理链路上以不同频率传输每个流,这与在无线电波上或在同轴有线电视链路上以不同频率传输不同电视台的信号大致相同. 
...
使用文件访问ㄡ数字图书馆和介绍中描述的两个视频应用程序 (视频会议和视频点播) 作为代表性示例,我们可能决定提供以下两种类型的频道: *请求/应答*渠道和*消息流*渠道. 请求/应答信道将被文件传输和数字图书馆应用所使用. 它将保证一方发送的每条消息都由另一方接收,并且每个消息的副本仅被传递. 请求/应答通道还可保护流经其上的数据的隐私和完整性,使得未经授权的当事方不能读取或修改在客户端和服务器进程之间交换的数据. 
